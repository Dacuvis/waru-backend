...
export class PromoService {
  ...
  async getById(id: string) {
    ...
    if (!promo) throw new AppError(`Promo dengan id ${id} tidak ditemukan`, 404);
  ...
  async applyPromo(data: ApplyPromo) {
    ...
    if (!promo) throw new AppError(`Kode promo "${data.code}" tidak ditemukan`, 404);

    const now = new Date();
    const promoData = promo as any;

    if (promoData.status !== "active") throw new AppError("Promo tidak aktif", 400);
    if (now < new Date(promoData.startDate)) throw new AppError("Promo belum dimulai", 400);
    if (now > new Date(promoData.endDate)) throw new AppError("Promo sudah berakhir", 400);
